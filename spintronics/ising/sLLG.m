...
Ms = 1.2e5; % A/m, Saturation Magnetization [emu/cm^3] emu/cm^3 = 1 KA/m  1.2e6
...
%%%%%%%%%%%%%%%%%%%72.864%%%%%%%%%%%%%%%%%%%%%%%%%%%%
P=0.33;
V_vcma=1.058;                        %1.05984 HVCMA=HK Voltage across the MTJ,[V]
...
for i=1:1:tot1
    Time(i)=i*step;
%     if(rem(i,100/step)==0)            %VCMA annealing
%         Vs=Vs-0.0002;
%         H_VCMA=2*ksi*Vs/(mu0*Ms*d_MgO*t_fl)
%     end
%     if(i<40000/step)
%         if(rem(i,200/step)==0)              %temperature annealing
%             T=T-1;
%             T
%         end
%     end
    ...
    if i==1
        ...
        m(i,3)=cos(theta(i));
%         m(i,:)=sLLG_step(I, m(i-1,:), Hth, V_vcma, delta_t);
    end
    %count
    if(m(i,3)>0)
...
pos_probability=m_positive/(m_positive+m_negtive);
pos_probability
figure;
hold on;
